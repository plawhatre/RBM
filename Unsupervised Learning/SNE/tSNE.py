...
class tSNE:
	...
	def Pj_and_i(self):
		return (self.P_conditional + self.P_conditional.T) / (2 * self.N)

	def Qj_and_i(self, Y, i, j):
		num = 1 / (1 + np.linalg.norm(Y[i,:]-Y[j,:])**2)
	...
	def forward(self):
		...
		for k in range(self.N):
			den += np.sum(
				1 / (1 + np.linalg.norm(Y[k,:]-np.delete(Y, k, axis=0),ord='fro', axis=(0,1))**2))
		self.normlization_Q = den

		for i in range(self.N):
			for j in range(self.N):
				self.P_conditional = self.Pj_given_i(self.X, i, j, self.sigma_2)
				self.Q[i,j] = self.Qj_and_i(self.Y, i, j)

		self.P[i,j] = self.Pj_and_i()
	...
	def grad(self):
		...
		E = 1 / (1 + Y_tilda**2)
		gradient = 4*np.sum(np.multiply(M, np.multiply(Y_tilda, E)), axis=0).reshape(np.shape(self.Y))
		return gradient

	def gradient_update(self, lr=0.001):
		self.forward()
		self.Y -= lr*4*self.grad()
	...
	def fit_transform(self, X, epochs=5, lr=0.005):
		...
		self.P_conditional = np.zeros(shape=(self.N, self.N), dtype=np.float32)
